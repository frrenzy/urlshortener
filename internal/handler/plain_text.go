...
func (s plainHandler) createShort(ctx context.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		short, err := s.URLService.CreateShortURL(ctx, *original)
		if err != nil {
...
func (s plainHandler) redirectToOriginal(ctx context.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		original, err := s.URLService.GetOriginalURL(ctx, short)
		if err != nil {
...
func newPlainRouter(ctx context.Context, services Services) chi.Router {
	router := chi.NewRouter()
	plainHandlers := plainHandler{Services: services}

	router.Route(`/`, func(r chi.Router) {
		r.Get(`/{id}`, plainHandlers.redirectToOriginal(ctx))
		r.Post(`/`, plainHandlers.createShort(ctx))
	})
